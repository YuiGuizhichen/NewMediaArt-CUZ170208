...
void draw() {
  ...
  background(255);
  //balls.vel;
  float noiseX = map(noise(t * 0.5), 0, 1, -lightness/100, lightness/100);
  float noiseY = map(noise(t + 1), 0, 1, -lightness/100, lightness/100);
...
void serialEvent (Serial myPort) {
  ...
  if (inString != null) {
    ...
    lightness = float(inString);           // convert to a number.
    lightness = map(lightness, 20, 60, 30, lightness); //map to the screen height.
    newData = true;
  }
}
